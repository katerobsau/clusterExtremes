# Example 1
grid_info = grid_classify %>% filter(k == 28) %>% select(x,y,class_id) %>%
  mutate(z = class_id %>% as.numeric(), xc = x, yc=y) %>%
  select(-class_id)
# expand.grd = expand.grid(xc = sort(unique(grid_info$x)),
#                          yc = sort(unique(grid_info$y))) %>%
#   as.data.frame()
# grd = full_join(grid_info, expand.grd)
...
class_plot <- ggplot() +
  ...
  geom_polygon(data = map.df, aes(long,lat, group = group), fill = "white",
               color= "blue") +
