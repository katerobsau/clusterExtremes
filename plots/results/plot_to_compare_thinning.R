...
for(i in 1:4){
  cluster_list[[i]] = pam(DD_list[[i]], k = k, diss = TRUE, medoids = NULL)
}
...
thin_name = c("No Thin", "Thin 10", "Thin 20", "Thin 30")
for(i in 1:4){
...
thin_name = c("No Thin", "Thin 10", "Thin 20", "Thin 30")
for(i in 1:4){
  df = data.frame(coord_list[[i]], cluster_id = hcluster_list[[i]]) %>%
    mutate(distance_type = thin_name[i])
  hplot_list[[i]] = df
}
...
mainland_df <- utils_mainland()
tas_df <- utils_tasmania()
hplot_title = paste(cluster_method, " with K =", k, " and linkage ", linkage_method, sep ="")
...
#
# # save out the plots
# pdf(file = thin_medoids_path, width = 5, height = 5)
# cluster_plot
# dev.off()
#
# pdf(file = thin_hclust_path, width = 5, height = 5)
# hcluster_plot
# dev.off()
